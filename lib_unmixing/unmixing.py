...
def vca(Y, R, verbose=True, snr_input=0):
    ...
    if snr_input == 0:
        ...
        Ud = splin.svd(np.dot(Y_o, Y_o.T) / float(N))[0][
            :, :R
...
def sunsal(M, y, **kwargs):
    ...
    if np.sum(Lambda == 0) and addone and not positivity:
        ...
        if lin.cond(F) > SMALL:
            ...
            IF = lin.inv(F)
            z = IF.dot(M.T).dot(y) - IF.dot(B.T).dot(lin.inv(B.dot(IF).dot(B.T))).dot(
                B.dot(IF).dot(M.T).dot(y) - a
            )
            # primal and dual residues
            res_p = 0
            res_d = 0
